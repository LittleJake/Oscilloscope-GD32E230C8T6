...
void Key_Handle(volatile struct Oscilloscope *value)
{
    ...
    float tempValue=0;
		...
		//set interval value for pwm change.
	  float gapValue = 250.0f;

	switch((*value).keyValue)
	{
		case KEY1:
            (*value).pwmOut=((*value).timerPeriod*0.02f)+(*value).pwmOut;
        ...
        case KEY3:
            ...
            tempValue=(*value).pwmOut/((*value).timerPeriod+0.0f);
				     if((*value).timerPeriod <= 1000) 
             {
				    ...
				    (*value).timerPeriod = (*value).timerPeriod - gapValue;
						if((*value).timerPeriod < 50)
            {
								(*value).timerPeriod = 10000;
            }
