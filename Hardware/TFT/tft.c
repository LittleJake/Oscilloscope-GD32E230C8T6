...
void TFT_StaticUI(void)
{
    ...
    TFT_ShowChinese(10,0,(uint8_t *)"简易示波器",RED,WHITE,16,0);
    
    sprintf(showData," PWM ");
    TFT_ShowString(110,0,(uint8_t *)showData,WHITE,RED,16,0);  
    memset(showData,0,32);
    
    TFT_ShowChinese(110,20,(uint8_t *)"输出状态",BLACK,WHITE,12,0);
    
//    sprintf(showData,"      ");
//    TFT_ShowString(110,36,(uint8_t *)showData,WHITE,RED,16,0);  
//    memset(showData,0,32);
    
    TFT_ShowChinese(110,56,(uint8_t *)"输出频率",BLACK,WHITE,12,0);
    
    sprintf(showData,"      ");
    TFT_ShowString(110,72,(uint8_t *)showData,WHITE,RED,16,0);  
    memset(showData,0,32);
    
    sprintf(showData,"        ");
    TFT_ShowString(110,92,(uint8_t *)showData,BLACK,WHITE,12,0);  
    memset(showData,0,32);
    TFT_ShowChinese(115,92,(uint8_t *)"占空比",BLACK,WHITE,12,0);
    
    sprintf(showData,"      ");
    TFT_ShowString(110,106,(uint8_t *)showData,WHITE,RED,16,0);  
    memset(showData,0,32);
    
    TFT_ShowChinese(5,96,(uint8_t *)"输入幅值",BLACK,WHITE,12,0);
    
    TFT_ShowChinese(55,96,(uint8_t *)"输入频率",BLACK,WHITE,12,0);
    
    for(i=0;i<=128;i=i+2)
    {
        TFT_DrawPoint(106,i,BLACK);
    }
    ...
    for(i=0;i<106;i++)
    {
			  if (i < 100) 
				{
						TFT_DrawPoint(i,90,BLACK);
				}
				//up and buttom line
				TFT_DrawPoint(i,19,BLACK);
				TFT_DrawPoint(i,94,BLACK);
    }
		
    for(i=20;i<=90;i++)
    {
        TFT_DrawPoint(0,i,BLACK);
    }
    for(i=0;i<10;i++)
    {
        TFT_DrawPoint((i*10)+2,91,BLACK); 
        TFT_DrawPoint((i*10)+3,91,BLACK); 
    }
    for(i=0;i<10;i++)
    {
        TFT_DrawPoint((i*10)+2,92,BLACK); 
        TFT_DrawPoint((i*10)+3,92,BLACK);
    }
}
/*
*   函数内容：  显示字符串
...
void TFT_ShowUI(volatile const struct Oscilloscope *value)
{
    ...
    TFT_ShowString(5,112,(uint8_t *)showData,WHITE,RED,16,0);  
    memset(showData,0,32);
    
    if((*value).gatherFreq>=1000)
    {
        sprintf(showData,"%2.0fKHz",(*value).gatherFreq/1000.0f);
        TFT_ShowString(55,112,(uint8_t *)showData,WHITE,RED,16,0);  
        memset(showData,0,32);
    }
    else
    {
    ...
    {   
        TFT_ShowString(110,36,(uint8_t *)"  ON  ",WHITE,GREEN,16,0);
    }
    else
    {
        TFT_ShowString(110,36,(uint8_t *)"  OFF ",WHITE,RED,16,0);     
    }
